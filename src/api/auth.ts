...
export const authRoutes = new Elysia({ prefix: "/api/auth" })
    .get("/github", ({ set, cookie }) => {
        ...
        logger.info("OAuth redirect", { redirectUrl });
    ...
    .get("/callback", async ({ query, set, cookie }) => {
        ...
        if (
            ...
            cookie.state?.remove();
            return doRedirect("/?error=invalid_request", set);
        }

        const savedState = cookie.state?.value;
        if (!savedState || savedState !== state) {
            cookie.state?.remove();
            return doRedirect("/?error=invalid_state", set);
        }
        ...
        try {
            ...
            if (!tokenData.access_token) {
                ...
                return doRedirect("/?error=token_exchange_failed", set);
            }
            ...
            return doRedirect("/", set);
        } catch (err) {
            logger.error("OAuth callback error", { error: String(err) });
            return doRedirect("/?error=auth_failed", set);
        }
    })
    ...
    .post("/logout", async ({ cookie }) => {
        if (cookie.session?.value) {
            const session = await validateSession(
                cookie.session.value as string,
            );
            if (session) {
                const db = getDb();
