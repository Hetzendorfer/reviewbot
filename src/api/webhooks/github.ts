...
export async function handleGitHubWebhook(
    ...
    console.log("Request was recognised");

    if (
        !verifyWebhookSignatureFn(
            rawBody,
            signature,
    ...
    recordWebhookTrace({
        ...
        prNumber:
            "number" in payload && typeof payload.number === "number"
                ? payload.number
    ...
    if (isPullRequestEventFn(event, payload as PullRequestEvent)) {
        ...
        try {
            ...
            if (
                ...
                loggerInstance.warn(
                    "Webhook rejected because review queue is not ready",
                    {
                        installationId: payload.installation.id,
                        repo: payload.repository.full_name,
                        pr: payload.number,
    ...
    if (
        ...
        try {
            ...
            if (
                ...
                loggerInstance.warn(
                    "Mention-triggered review rejected because queue is not ready",
                    {
                        installationId: payload.installation.id,
                        repo: payload.repository.full_name,
        ...
        try {
            ...
            loggerInstance.warn(
                "Failed to add eyes reaction to review request comment",
                {
                    installationId: payload.installation.id,
                    repo: payload.repository.full_name,
                    pr: payload.issue.number,
